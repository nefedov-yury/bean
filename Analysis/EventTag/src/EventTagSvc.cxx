...
EventTagSvc::EventTagSvc () {
   m_pdtFile = "Analysis/EventTag/share/pdt_bean.table";
...
#else
   if( m_isinit ) {
      cout << " The service has already been initialized "
           << endl;
      return false;
   }
   if( verbose ) {
...
int EventTagSvc::readDecayTabFile(string fname){
  ...
  while (! infile.eof() ){
    ...
    if(ln.size()>255){
#ifndef BEAN
      log<<MSG::ERROR<<"line "<<nstr<<" in "<<fname<<" is too big"<<endreq;
#else
...
//           nneu,ncha);
	 ...
	 while (TMcParticle * particle =
	    ...
	    if (pdg == -15) {
         ...
         delete mcParticlesIter5;
      }

      delete mcParticlesIter4;
   }
