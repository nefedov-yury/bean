...
def IsLocked():
    try:
        urllib.request.urlopen(HOST+"/LOCK")
        return True
...
def GetTimestamp():
    ...
    for line in lines[1:]:
        ...
        Files[temp2]=temp1
#      print(" GetTimestamp return: ",timestamp,Files)
...
def SelectDbForUpdate(timestamp,Files):
    ...
    if os.path.exists("db.timestamp"):
        ...
        if not lines[0].strip().isdigit():
            print("File db.timestamp is broken." \
        ...
        if cur_timestamp == timestamp:
            ...
            print("Warning! Please check: current database is NEWER" \
                  " than the remote replica. Do nothing.")
            return LoadList
...
def DownloadDb(dbname):
    ...
    try:
        ...
        for i in range(10):
            if IsLocked():
                var=""
                if i>0: var="\r"
                sys.stdout.write(var+"Lock file found." \
                    " It looks like server replica is being updated." \
                    " Sleeping for "+str(i+1)+" min")
                sys.stdout.flush()
                time.sleep(1)
            elif i>0:
        ...
        if i==9:
            print("\nTimeout: 10 min waiting to unlock the server." \
                  " Please try again later, or complain.")
            exit(-1)
    ...
    if ret != 0 or \
       not os.path.exists(tmpdbname) or \
       os.path.getsize(tmpdbname)==0 :
...
def main(argv):
    ...
    if destdir=="":
        print("Destination directory neither given explicitly," \
...
#              print(" CHECK: ",tmpdbname,dbname,chksum)
