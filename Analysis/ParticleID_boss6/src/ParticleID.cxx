...
void ParticleID::init() {
   ...
   if(IsMrpcInfoUsed()) {
      if(!m_mrpcpid) m_mrpcpid = MrpcPID::instance();
      m_mrpcpid->init();
   }
...
void ParticleID::calculate(int run)
{
   ...
   if(m_pidsys == 0) {
     m_pidsys = useDedx() | useTof() | useTofE() | useEmc() | useMuc() | useTofQ() | useTofC() | useTofCorr() | useMrpc();
   }
   ...
   */
   ...
   if(IsMrpcInfoUsed()) {
      if(!m_mrpcpid) m_mrpcpid = MrpcPID::instance();
      m_mrpcpid->init();
      m_mrpcpid->setRecTrack(recTrk);
      m_mrpcpid->setChiMinCut(chiMinCut());
      m_mrpcpid->setPdfMinSigmaCut(pdfMinSigmaCut());
      m_mrpcpid->calculate();
   }
...
int ParticleID ::particleIDCalculation() {
   ...
   bool valid = IsDedxInfoValid() || IsTofInfoValid()||IsTofEInfoValid()
     ...
     || IsTofCInfoValid() || IsTofCorrInfoValid() || IsMrpcInfoValid();
   ...
   if(IsMrpcInfoUsed()) {
      if(IsMrpcInfoValid()) {
         bool okpid = false;
         for(int i = 0; i < 5; i++) {
            if(pidcase[i] && (fabs(chiMrpc(i)) < m_mrpcpid->chiMinCut()))
               if(!okpid) okpid = true;
         }
         if(okpid) {
            m_ndof++;
            for(int i = 0; i < 5; i++) chisq[i] += chiMrpc(i)*chiMrpc(i);
         }
      } // Mrpc Pid
   }
...
int ParticleID::LikelihoodCalculation() {
   ...
   bool valid = IsDedxInfoValid() || IsTofInfoValid() || IsTofEInfoValid() || IsTofQInfoValid() || IsTofCInfoValid() || IsTofCorrInfoValid() || IsEmcInfoValid()||IsMucInfoValid() || IsMrpcInfoValid();
   if(!valid) return irc;
   double pdf[5];
   bool pidcase[5];
   for(int i = 0; i < 5; i++) {
