...
public:
 ...
 // Double_t prob(Int_t i)  const { return m_prob[i]; }
  ...
  Double_t dedxExpect(int pid) const { return m_dedx_exp[pid-1];   }
  Double_t sigmaDedx(int pid) const { return m_sigma_dedx[pid-1];   }
  Double_t pidProb(int pid) const { return m_pid_prob[pid-1];   }
